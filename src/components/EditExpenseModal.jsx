...
import { updateExpense } from '../supabaseService';
...
const EditExpenseModal = ({ isOpen, onClose, expense, onExpenseUpdated }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    setError(null);
    try {
      await updateExpense(expense.id, expenseFormData);
      onExpenseUpdated();
      onClose();
    } catch (err) {
      setError(err);
  ...
  return (
    <div className="fixed inset-0 bg-dark-overlay flex justify-center items-center backdrop-blur-sm z-50">
      <div className="bg-dark-secondary bg-opacity-90 backdrop-blur-md rounded-xl p-8 w-full max-w-md shadow-lg border border-accent-premium">
        <h2 className="text-2xl font-semibold text-light-primary mb-6 font-display">Editar Gasto</h2>
