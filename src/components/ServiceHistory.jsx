...
const ServiceHistoryComponent = () => { // Renamed component to avoid confusion with ServiceModal
      ...
      {isServiceModalOpen && (
        <div className="fixed inset-0 bg-dark-overlay flex justify-center items-center backdrop-blur-sm">
          <div className="bg-dark-secondary bg-opacity-90 backdrop-blur-md rounded-xl p-8 w-full max-w-2xl shadow-lg border border-accent-premium" style={{ marginTop: '20px', marginBottom: '70px', overflowY: 'auto', maxHeight: 'calc(100vh - 140px)' }}>
            ...
            <ServiceModalComponent // Renamed to ServiceModalComponent to avoid naming conflict
...
export default ServiceHistoryComponent; // Export the renamed component


export const ServiceModal = ({ isOpen, onClose, onSubmit, serviceFormData, setServiceFormData, clients, motorcycles, inventory, currentService }) => { // Export ServiceModal
  ...
  const filteredClients = useMemo(() => clients.filter(client => client.name.toLowerCase().includes